\documentclass[10pt,journal,a4paper]{IEEEtran}
%\documentclass[letterpaper,twocolumn,10pt]{article}

\usepackage{amsmath}                        % extended math env
\usepackage{amssymb}                        % more math symbols
% \usepackage{balance}                      % balance coloumns on final page
\usepackage{booktabs}                       % nicer tables + hline thickness
% \usepackage{caption}                        % control of caption sizing
\usepackage[usenames,dvipsnames]{color}     % color access

\usepackage{comment}
\let\labelindent\relax
\usepackage[inline]{enumitem}

\usepackage{listings}
\usepackage[inline]{enumitem}
\usepackage[bottom]{footmisc}
\usepackage[noadjust]{cite}
\usepackage{graphicx}
\graphicspath{ {Figures/} }
\usepackage{caption}



\newcommand{\adrien}[1]{\noindent{\color{Green} {\bf \fbox{AG} {\it#1}}}}
% *** CITATION PACKAGES ***
%
\ifCLASSOPTIONcompsoc
  % IEEE Computer Society needs nocompress option
  % requires cite.sty v4.0 or later (November 2003)
  % \usepackage[nocompress]{cite}
\else
  % normal IEEE
  % \usepackage{cite}
\fi


% *** GRAPHICS RELATED PACKAGES ***
%
\ifCLASSINFOpdf
  % \usepackage[pdftex]{graphicx}
  % declare the path(s) where your graphic files are
  % \graphicspath{{../pdf/}{../jpeg/}}
  % and their extensions so you won't have to specify these with
  % every instance of \includegraphics
  % \DeclareGraphicsExtensions{.pdf,.jpeg,.png}
\else
  % or other class option (dvipsone, dvipdf, if not using dvips). graphicx
  % will default to the driver specified in the system graphics.cfg if no
  % driver is specified.
  % \usepackage[dvips]{graphicx}
  % declare the path(s) where your graphic files are
  % \graphicspath{{../eps/}}
  % and their extensions so you won't have to specify these with
  % every instance of \includegraphics
  % \DeclareGraphicsExtensions{.eps}
\fi

\providecommand{\PSforPDF}[1]{#1}


% NOTE: PDF hyperlink and bookmark features are not required in IEEE
%       papers and their use requires extra complexity and work.
% *** IF USING HYPERREF BE SURE AND CHANGE THE EXAMPLE PDF ***
% *** TITLE/SUBJECT/AUTHOR/KEYWORDS INFO BELOW!!           ***
\newcommand\MYhyperrefoptions{bookmarks=true,bookmarksnumbered=true,
pdfpagemode={UseOutlines},plainpages=false,pdfpagelabels=true,
colorlinks=true,linkcolor={black},citecolor={black},pagecolor={black},
urlcolor={black},
pdftitle={OS Design When the OS Cannot Be Trusted},%<!CHANGE!
pdfsubject={OS},%<!CHANGE!
pdfauthor={Adrien Ghosn},%<!CHANGE!
pdfkeywords={OS, Exokernel, Unikernel, SGX, Haven, Cloud}}%<^!CHANGE!



% correct bad hyphenation here
\hyphenation{}


\begin{document}

\title{OS Design When the OS Cannot Be Trusted}
\author{Adrien Ghosn \\
        DCSL, I\&C, EPFL\\
        \today}





% The paper headers
\markboth{EDIC Research Proposal}%
{Shell \MakeLowercase{\textit{et al.}}: EDIC Research Proposal}

\IEEEcompsoctitleabstractindextext{%

\input{abstract}

\begin{IEEEkeywords}
kernel design, exokernel, unikernel, SGX, Haven, security, performance, Cloud deployment
\end{IEEEkeywords}}

% make the title area
\maketitle

\IEEEdisplaynotcompsoctitleabstractindextext

\IEEEpeerreviewmaketitle



%\section{Introduction}
\input{introduction2}

\input{exokernel}

\input{unikernels}

\input{haven}

\input{research_proposal_v2}

\newpage
{%
  \frenchspacing
  \bibliography{bibliography}
  \bibliographystyle{ieeetr}
}
%\bibliography{bibliography}
%\bibliographystyle{acm}




% that's all folks
\end{document}


